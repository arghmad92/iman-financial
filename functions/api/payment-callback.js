...
async function sendConfirmationEmail({ env, to, name, orderId }) {
  ...
  // Plain-text version — improves deliverability and serves clients without
  // HTML rendering (some screen readers, plain-text mode, etc.)
  const greetingName = name ? `, ${name}` : '';
  const refLine = orderId ? `\nReference:  ${orderId}` : '';
  const zoomText = zoomUrl
    ? `\nJOIN THE WEBINAR\n${zoomUrl}\n`
    : `\nZoom link will be sent in a separate email closer to the event date.\n`;

  const text = `You're in${greetingName}.
...
Date:       22 May 2026
Time:       8:00 PM (MYT)
Platform:   Zoom${refLine}
${zoomText}
Save the date and add it to your calendar. We'll send a reminder one day
before. If you have any questions, just reply to this email.
